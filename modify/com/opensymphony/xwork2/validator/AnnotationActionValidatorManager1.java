 ...
 * @author jepjep
 */
public abstract class AnnotationActionValidatorManager1 implements ActionValidatorManager {
		...
		for (final Validator validator : validators) {
			try {
				...
				if (validator instanceof FieldValidator) {
					...
					fullFieldName = new InternalValidatorContextWrapper( fValidator.getValidatorContext() ).getFullFieldName( fValidator
							.getFieldName() );
	...
	private List<ValidatorConfig> buildValidatorConfigs(Class clazz, String context, boolean checkFile, Set<String> checked) {
		...
		if (context != null) {
			...
			validatorConfigs.addAll( buildAliasValidatorConfigsJSON( clazz, context, checkFile, fileManager, validatorFileCache, validatorFactory,
					LOG ) );
		}
	...
	protected abstract List<ValidatorConfig> buildAliasValidatorConfigsJSON(Class clazz, String context, boolean checkFile, FileManager fileManager,
